...
class Layer {
  ...
  show(again) {
    if (!this.visible) {
      if (this.middle) {
        this.modal.getElement().addClass('middle');
      }
  ...
  hide() {
    if (this.middle) {
      this.modal.getElement().removeClass('middle');
    }
    if (this.visible) {
      this.getElement().removeClass("in").addClass("out");
    }
    this.visible = false;
  }
  destroy() {
  ...
  resize(size) {
    if (this.$element) {
      if (this.size && this.size !== size) {
        this.$element.removeClass('layer-' + this.size);
      }
      if (size) {
        this.$element.addClass('layer-' + size);
      }
    }
    this.size = size;
    return this;
