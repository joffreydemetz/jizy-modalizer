...
Modalizer.isShown = function () { return this.shown; };

Modalizer.addLayer = function (name, config, replace) {
  if (replace) { this.layers.pop().destroy(); }
  ...
  if (config.onShow) { layer.setOnShow(config.onShow); }
  if (config.onHide) { layer.setOnHide(config.onHide); }
